...
def ejecutar_clips_vocacional(respuestas):
    ...
    for fact in env.facts():
        fact_str = str(fact)
        if "es-" in fact_str:  # Busca el hecho que indica el diagnóstico
            # Elimina cualquier paréntesis alrededor del hecho
            resultado = fact_str.replace("(es-", "").replace(")", "").strip()
            resultados.append(resultado.capitalize())

    # Invertir los resultados (FIFO: el primero en entrar es el primero en salir)
    resultados.reverse()

    if resultados:
        # Aquí enumeramos después de invertir los resultados para asignar los números correctos
        opciones = "\n".join([f"OPCION {i+1}: {resultado}" for i, resultado in enumerate(resultados)])
        messagebox.showinfo("Resultado", f"Los resultados de la orientación vocacional son:\n{opciones}")
    else:
...
ventana = tk.Tk()
ventana.title("Sistema Experto de Orientación Vocacional")
ventana.geometry("1000x900")
ventana.configure(bg="#e3f2fd")  # Fondo azul claro

# Cargar y mostrar la imagen en la interfaz
image = Image.open("vocacion.jpeg")  
image = image.resize((300, 200))  
photo = ImageTk.PhotoImage(image)  
image_label = tk.Label(ventana, image=photo, bg="#e3f2fd")
image_label.image = photo  
image_label.pack(pady=5)

# Título y descripción
titulo_label = tk.Label(ventana, text="Identificador de Intereses Vocacionales", font=("Arial", 16, "bold"), bg="#e3f2fd", fg="#0d47a1")
titulo_label.pack(pady=10)

descripcion_label = tk.Label(ventana, text="Selecciona tus principales intereses y presiona 'Determinar Vocación'.", 
                             font=("Arial", 10), bg="#e3f2fd", fg="#333333")
...
tk.Label(ventana, text="Selecciona tus principales intereses:", font=("Arial", 12), bg="#e3f2fd", fg="#333333").pack(pady=10)

checkbutton_frame = tk.Frame(
    ventana,
    bg="#bbdefb",         # Color suave de fondo para el frame
